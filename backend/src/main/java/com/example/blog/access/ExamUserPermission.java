...
/**
 * Direct, per-user grant to a single private exam — the escape hatch for
 * exceptions that don't warrant creating/joining a whole access group.
 * Mirrors PostUserPermission.
 */
@Entity
@Table(
    name = "exam_user_permissions",
    uniqueConstraints = @UniqueConstraint(columnNames = {"exam_id", "user_id"})
)
public class ExamUserPermission {
